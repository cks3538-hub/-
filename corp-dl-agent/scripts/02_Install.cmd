...
setlocal
rem ---------------------------------------------------------------------------
rem  02_Install: venv in releases\<release_id>\<profile_id>\ -> offline pip (no-index, require-hashes) -> self-test -> active.json
...
echo [02] install
%PYCMD% "%SCRIPT_DIR%install.py" --package "%PKG_DIR%" --install-root "%DIA_INSTALL_ROOT%" %DATA_ARG% %*
set "EC=%ERRORLEVEL%"
