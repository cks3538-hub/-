...
setlocal
rem ---------------------------------------------------------------------------
rem  06_Rollback: switch active.json back to a previous release (code rollback; DB restore only with --restore-db)
...
echo [06] rollback
if "%~1"=="" (
  echo Usage: 06_Rollback.cmd --to ^<release_id^> [--restore-db ^<backups\timestamp^>]
  set "EC=2"
  goto :end
)
