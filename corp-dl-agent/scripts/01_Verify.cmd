...
setlocal
rem ---------------------------------------------------------------------------
rem  01_Verify: manifest / file count / size / sha256 / path traversal / target OS-Python-ABI checks
...
echo [01] verify
%PYCMD% "%SCRIPT_DIR%verify.py" --package "%PKG_DIR%" --install-root "%DIA_INSTALL_ROOT%" %*
set "EC=%ERRORLEVEL%"
