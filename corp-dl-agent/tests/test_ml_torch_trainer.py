...
from test_ml_torch_common import FakeClock, make_cfg, prepared_data, read_jsonl, running_coordinator, small_task, tracker
...
def _train(tmp_path: Path, coord: Any, data: Any, *, epochs: int = 3, resume: bool = False, hooks: TrainerHooks | None = None, budget: Any = None, fingerprint: str = "fp-1", **kw: Any) -> Any:
...
def test_train_completes_with_finite_metrics(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert is_finite_metrics({k: v for k, v in res.validation_metrics.items() if k in ("mae", "rmse", "r2", "p95_abs_error")})
        assert res.validation_metrics["unit"] == "N" and res.device == "cpu" and res.amp is False and res.seed == 42
        run_dir = tmp_path / "runs" / RUN_ID
...
def test_classification_trains_and_picks_threshold(tmp_path: Path, cls_data: Any) -> None:
    ...
    try:
        ...
        assert is_finite_metrics({k: m[k] for k in ("average_precision", "roc_auc", "f1", "precision", "recall")})
...
def test_oom_injection_creates_new_attempts_with_recorded_changes(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert [c["microbatch"] for c in res.attempt_changes] == [{"from": 64, "to": 32}, {"from": 32, "to": 16}]
        assert [c["accumulation"] for c in res.attempt_changes] == [{"from": 1, "to": 2}, {"from": 2, "to": 4}]
        ...
        assert [a.status for a in attempts] == [AttemptStatus.FAILED, AttemptStatus.FAILED, AttemptStatus.COMPLETED]
        assert attempts[0].reason == "oom_simulated" and attempts[1].changes["microbatch"] == {"from": 64, "to": 32}
...
def test_nan_injection_halves_lr_once(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert res.attempt_changes[0]["learning_rate"] == {"from": lr0, "to": lr0 / 2} and res.learning_rate == lr0 / 2
        ...
        attempts = coord.list_attempts(res.trial_id)
        assert attempts[0].reason == "non_finite_loss_or_grad" and attempts[1].changes["reason"] == "non_finite"
        rows = read_jsonl(tmp_path / "runs" / RUN_ID / "epoch_metrics.jsonl")
        assert [r["epoch"] for r in rows] == [0, 1]
        res2 = _train(tmp_path, coord, reg_data, epochs=2, hooks=NanHooks({1, 2}), fingerprint="fp-2", trial_id="run-t-nan2")
...
def test_pause_at_epoch_boundary_then_resume_continues(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert res2.status == "COMPLETED" and res2.epochs_completed == 5 and res2.resumed_from_epoch == 2 and res2.attempts == 2
        rows = read_jsonl(tmp_path / "runs" / RUN_ID / "epoch_metrics.jsonl")
...
def test_forced_kill_after_checkpoint_then_resume(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert res.status == "COMPLETED" and res.resumed_from_epoch == 2 and res.epochs_completed == 4 and res.attempts == 2
        attempts = coord.list_attempts(trial_id)
        assert attempts[0].status is AttemptStatus.ABORTED and attempts[0].reason == "superseded by new attempt"
        rows = read_jsonl(tmp_path / "runs" / RUN_ID / "epoch_metrics.jsonl")
...
def test_corrupt_latest_falls_back_to_best_and_both_corrupt_fails(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        assert res2.status == "COMPLETED" and res2.resumed_from_epoch == best_next and res2.epochs_completed == 6
        ...
        res3 = _train(tmp_path, coord, reg_data, epochs=4, hooks=PauseHooks(at_epoch=1), fingerprint="fp-3", trial_id="run-t-corrupt")
        ...
        with pytest.raises(AgentError) as ei:
            _train(tmp_path, coord, reg_data, epochs=4, resume=True, fingerprint="fp-3", trial_id="run-t-corrupt")
...
def test_budget_exceeded_stops_safely_and_can_resume(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        res3 = _train(tmp_path, coord, reg_data, epochs=4, hooks=ClockHooks(clock2, 60.0), budget=b2, fingerprint="fp-b2", trial_id="run-t-b2")
...
def test_resume_with_changed_fingerprint_rejected(tmp_path: Path, reg_data: Any) -> None:
    ...
    try:
        ...
        with pytest.raises(AgentError) as ei2:
            _train(tmp_path, coord, reg_data, epochs=3, resume=True, fingerprint="fp-1", config_hash="other-config")
