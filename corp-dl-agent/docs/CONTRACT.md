...
# trainer.py: train_candidate(candidate, data, budget: BudgetTracker, coordinator, run_dir, *, device="cpu", resume=True) -> TrialResult
...
# runner.py: run_task(spec, cfg, ws, *, run_id=None, resume=False) -> RunSummary  # environment.json, manifest.json, data_report, split_manifest, plan.json, events.jsonl, epoch_metrics.jsonl, checkpoints/, metrics.csv, final_evaluation.json, model_card.json, export/  ; 기준 모델 + MLP 후보 비교 → validation 으로 선택 → test 1회 평가(final_evaluation.json 있으면 재평가 금지) ; pause/resume/cancel 은 coordinator 통해
# predict.py: predict_cli(model_dir, input_csv, output_csv, cfg)
