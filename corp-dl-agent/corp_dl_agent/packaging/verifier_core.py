"""corp-dl-agent 반입 패키지 검증·설치 공통 코어 (표준 라이브러리만).

- 이 파일은 `corp_dl_agent/packaging/verifier_core.py` 와 byte 단위로 동일해야 한다
  (tests/test_scripts_stdlib.py 가 검사). 앱 쪽 packaging 모듈과 설치 스크립트가 같은 파서/검사기를 쓰도록
  하기 위한 구조이며, 스크립트는 앱이 설치되지 않은 상태(사내 PC 첫 실행)에서 동작해야 하므로
  corp_dl_agent 를 import 하지 않는다.
- 구문은 오래된 Python 에서도 "버전이 낮다" 는 진단이 나오도록 3.8 호환 범위로 유지한다.
