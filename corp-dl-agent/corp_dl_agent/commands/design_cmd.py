...
def register(sub: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    ...
    m.add_argument(
        "--scope", default=None, help="MassScope JSON (도장/폼/접착제/구매품 포함 범위와 명시 kg 항목)"
    )
    m.add_argument(
        "--material-table",
        default=None,
        help="재료표 CSV (config cad.material_table 대신). 정책이 use_material_table 일 때만 사용",
    )
    ...
    c.add_argument(
        "--occurrence", default=None, help="특정 occurrence_path 의 단위 질량 기준으로 계산 (기본: 총 질량)"
    )
    c.add_argument("--currency", default=None, help="기대 통화 (recipe 와 다르면 E_REVISION_MISMATCH)")
    c.add_argument(
        "--base-date", default=None, help="기대 기준일 YYYY-MM-DD (recipe 와 다르면 E_REVISION_MISMATCH)"
    )
    c.set_defaults(handler=run_cost)

    cp = sp.add_parser(
        "compare", help="후보 폴더(A/B/C)의 mass/cost/performance/constraint 비교 → design_comparison.json"
    )
...
def run_mass(args: argparse.Namespace) -> int:
    ...
    if args.scope:
        try:
            ...
            raise AgentError(
                "E_SCHEMA_INVALID", "MassScope JSON 이 올바르지 않습니다", details={"error": str(exc)[:500]}
            ) from exc
...
def run_cost(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        raise AgentError(
            "E_SCHEMA_INVALID",
            f"mass_result.json 이 schema 와 일치하지 않습니다: {mass_path.name}",
            details={"error": str(exc)[:500]},
        ) from exc
    recipes = load_recipes(check_input_path(cfg, args.recipes))
    if args.recipe not in recipes:
        raise AgentError(
            "E_INPUT_INVALID", f"recipe '{args.recipe}' 가 없습니다", details={"available": sorted(recipes)}
        )
    subject = mass.item(args.occurrence) if args.occurrence else mass
    breakdown = compute_cost(
        subject, recipes[args.recipe], expected_currency=args.currency, expected_base_date=args.base_date
    )
    out = check_output_path(cfg, args.output)
...
def run_compare(args: argparse.Namespace) -> int:
    ...
    cmp = compare_designs(
        candidates, require_same_revision_policy=not args.allow_revision_mismatch, baseline_id=args.baseline
    )
    out = check_output_path(cfg, args.output)
