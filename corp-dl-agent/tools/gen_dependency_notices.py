...
def read_metadata(whl: Path) -> tuple[dict[str, str], list[str], dict[str, str]]:
    with zipfile.ZipFile(whl) as zf:
        ...
        fields = {
            k: (msg.get(k) or "").strip()
            for k in ("Name", "Version", "License", "License-Expression", "Home-page", "Author", "Summary")
        }
        ...
        for n in zf.namelist():
            if n.startswith(distinfo + "/licenses/") or (
                n.startswith(distinfo + "/")
                and n.rsplit("/", 1)[-1].upper().startswith(("LICENSE", "COPYING", "NOTICE"))
            ):
                try:
                    license_files[n.rsplit("/", 1)[-1]] = zf.read(n).decode("utf-8", errors="replace")
                except Exception as exc:  # noqa: BLE001
                    license_files[n.rsplit("/", 1)[-1]] = f"(읽기 실패: {exc})"
        ...
        for whl in sorted(Path(wh).glob("*.whl")):
            ...
            entry = seen.setdefault(
                key,
                {
                    ...
                    "license": fields["License-Expression"]
                    or fields["License"]
                    or ("; ".join(classifiers) if classifiers else "UNKNOWN"),
                    "classifiers": classifiers,
                    "home": fields["Home-page"] or fields["_urls"],
                    "wheels": [],
            ...
            entry["wheels"].append(
                {
                    ...
                    "profile": Path(wh).name,
                }
            )
    lines = [
        "# DEPENDENCY_NOTICES",
        "",
        "재배포되는 third-party Python 패키지의 라이선스 고지. 출처: PyPI (wheel METADATA 기준). 회사 반입/사용 승인은 별도 절차이며 여기 기재는 승인을 뜻하지 않습니다.",
        "",
        "| 패키지 | 버전 | 라이선스 | 홈페이지/URL | wheel (profile) |",
        "|---|---|---|---|---|",
    ]
