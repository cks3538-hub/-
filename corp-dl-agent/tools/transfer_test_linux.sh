...
run_step transfer-03-verify   "$PY312 '$PKG/scripts/verify.py' --package '$TEST_ROOT/in/$ZIPNAME' --json > '$TEST_ROOT/verify.json'; rc=\$?; head -c 3000 '$TEST_ROOT/verify.json'; exit \$rc"
run_step transfer-03b-verify-tampered-must-fail "$PY312 '$PKG/scripts/verify.py' --package '$TEST_ROOT/in/tampered.zip' --json > '$TEST_ROOT/verify_tampered.json'; rc=\$?; head -c 2000 '$TEST_ROOT/verify_tampered.json'; echo exit=\$rc; test \$rc -ne 0"
run_step transfer-04-install  "$PY312 '$PKG/scripts/install.py' --package '$TEST_ROOT/in/$ZIPNAME' --install-root '$INSTALL_ROOT' --data-root '$DATA_ROOT' --python $PY312 --json > '$TEST_ROOT/install.json'; rc=\$?; tail -c 4000 '$TEST_ROOT/install.json'; exit \$rc"
run_step transfer-05-active   "cat '$INSTALL_ROOT/active.json' && ls '$INSTALL_ROOT/releases'"
run_step transfer-06-selftest "$PY312 '$PKG/scripts/selftest.py' --install-root '$INSTALL_ROOT' --data-root '$DATA_ROOT' --json > '$TEST_ROOT/selftest.json'; rc=\$?; tail -c 3000 '$TEST_ROOT/selftest.json'; exit \$rc"
run_step transfer-07-doctor   "$PY312 '$PKG/scripts/launch.py' --install-root '$INSTALL_ROOT' --data-root '$DATA_ROOT' doctor --json > '$TEST_ROOT/doctor.json'; rc=\$?; head -c 2500 '$TEST_ROOT/doctor.json'; exit \$rc"
run_step transfer-08-demo     "$PY312 '$PKG/scripts/launch.py' --install-root '$INSTALL_ROOT' --data-root '$DATA_ROOT' demo --offline --device cpu --output-dir '$DATA_ROOT/outputs/demo' --json > '$TEST_ROOT/demo.json'; rc=\$?; tail -c 4000 '$TEST_ROOT/demo.json'; exit \$rc"
run_step transfer-09-install-tampered-keeps-active "cp '$INSTALL_ROOT/active.json' /tmp/active.before; $PY312 '$PKG/scripts/install.py' --package '$TEST_ROOT/in/tampered.zip' --install-root '$INSTALL_ROOT' --data-root '$DATA_ROOT' --python $PY312 --json > '$TEST_ROOT/install_tampered.json'; rc=\$?; tail -c 1500 '$TEST_ROOT/install_tampered.json'; echo exit=\$rc; cmp /tmp/active.before '$INSTALL_ROOT/active.json' && test \$rc -ne 0"
