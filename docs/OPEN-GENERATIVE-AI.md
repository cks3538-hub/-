...
## 0단계. 먼저 이것부터 확인하세요 — 아무것도 안 해도 될 수 있습니다

**Claude Code에 Hugging Face 커넥터가 연결돼 있으면, 이미 이미지 생성이 됩니다.**
muapi 키도, 환경 설정도, 설치도 필요 없습니다.

확인 방법: Claude Code에 이렇게 말해보세요.

```
노을 지는 산속 호수 사진 하나 만들어줘
```

바로 이미지가 나오면 **여기서 끝입니다.** 아래는 읽지 않으셔도 됩니다.

쓸 수 있는 모델은 세 가지입니다. 전부 무료입니다.

| 모델 | 언제 쓰나 |
|---|---|
| **FLUX.1 Schnell** | 범용. 빠르고 무난합니다. 기본으로 쓰세요 |
| **FLUX.1 Krea Dev** | 제품 사진·인물처럼 **실사**가 필요할 때 |
| **Z-Image Turbo** | 21:9, 9:16 등 **비율·해상도**를 정확히 맞춰야 할 때 |

**muapi가 꼭 필요한 경우는 이것뿐입니다:**

- 🎬 **영상** 생성 (Kling, Veo, Sora 등)
- 🎙️ **립싱크**, 배경 제거, 업스케일, 얼굴 합성
- 🎵 **음악·효과음** 생성
- 🖼️ 기존 이미지 **부분 수정**, 참조 이미지 여러 장 입력

위 네 가지가 필요 없으시면 이 문서를 더 읽지 않으셔도 됩니다.
...
| 나에게 맞는 것 | 하는 일 | 할 단계 |
|---|---|---|
| **폰만 쓴다 (PC 없음)** | 브라우저에서 바로 생성. 설치·설정 없음 | [muapi.ai/open-generative-ai](https://muapi.ai/open-generative-ai) 접속 |
...
### 📱 폰(안드로이드·아이폰 Claude 앱)에서 쓰는 경우 — 먼저 읽으세요

**폰에서는 환경 설정을 바꿀 수 없습니다.** 환경 설정 화면(구름 아이콘 → 톱니바퀴)은
**PC 브라우저**와 **데스크톱 앱**에만 있습니다. 폰 앱에는 그 메뉴 자체가 없습니다.

그래서 폰만 쓰신다면 **Claude Code로 이미지를 만드는 건 불가능**합니다. 대신 아래 둘 중 하나를 쓰세요.

| 방법 | 설명 |
|---|---|
| **① muapi 웹사이트를 폰 브라우저에서 직접 사용** *(추천)* | 설치·설정 전혀 없음. 👉 [muapi.ai/open-generative-ai](https://muapi.ai/open-generative-ai) 접속 → 로그인 → 바로 생성 |
| **② PC 앞에 앉았을 때 아래 🅰️ 설정을 한 번만** | 그 뒤로는 폰에서도 Claude Code로 생성 가능 (설정은 계정에 저장됨) |

②를 하실 거면 PC 브라우저에서 아래를 따라하세요. **폰에서는 이 단계가 안 됩니다.**

---

### 🅰️ PC 브라우저(claude.ai/code) 또는 데스크톱 앱에서 설정하는 경우
...
| 증상 | 해결 |
|---|---|
| **claude.ai/code 접속이 안 됨 / 구름 아이콘이 없음** | 폰에서 접속하셨을 가능성이 큽니다. 환경 설정은 **PC 브라우저나 데스크톱 앱에만** 있습니다 |
